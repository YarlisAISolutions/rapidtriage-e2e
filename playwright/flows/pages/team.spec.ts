/**
 * Team Page — rapidtriage.me/team
 * Feature: Team member management
 * Run: npx playwright test playwright/flows/pages/team.spec.ts --project=chromium
 */
...
test.describe('Team page', () => {
  test('page returns 200 or redirects to login', async ({ page }) => {
    const res = await page.goto(`${BASE}/team`);
  ...
  test('team page has expected heading', async ({ page }) => {
  ...
  test('team page loads without fatal JS errors', async ({ page }) => {
    const errors: string[] = [];
  ...
  test('team page shows sidebar navigation', async ({ page }) => {
    await page.goto(`${BASE}/team`);
    await page.waitForLoadState('networkidle');
    if (page.url().includes('login')) {
      test.skip(true, 'Redirected to login — auth required');
    }
    await expect(page.locator(Selectors.sidebar.navTeam)).toBeVisible({ timeout: 10_000 });
  });
});
